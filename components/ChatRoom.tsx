...
const ChatRoom: React.FC<ChatRoomProps> = ({ room, currentUser, onLeave, onSendMessage }) => {
  ...
  return (
    <div className="w-full max-w-5xl h-full max-h-[90vh] flex flex-col p-0 bg-black/30 backdrop-blur-xl rounded-xl shadow-2xl overflow-hidden border border-cyan-400/30 shadow-cyan-500/20">
      <header className="flex items-center justify-between p-4 bg-black/20 border-b border-white/20 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {room.topic && <p className="text-sm text-cyan-300 flex items-center gap-1"><SparklesIcon className="w-4 h-4" /> {room.topic}</p>}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {room.messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 items-end ${msg.user.id === currentUser.id ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-xs md:max-w-md lg:max-w-lg p-3 rounded-xl ${msg.user.id === currentUser.id ? 'bg-gradient-to-br from-cyan-500 to-blue-600 rounded-br-none' : 'bg-slate-800 rounded-bl-none'}`}>
              <p className={`text-sm font-bold ${msg.user.id === currentUser.id ? 'text-white/90' : 'text-cyan-300'}`}>{msg.user.id === currentUser.id ? 'Você' : msg.user.name}</p>
      ...
      <footer className="p-4 bg-black/30 border-t border-white/20">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-grow px-4 py-3 text-white bg-black/40 border border-slate-600 rounded-full focus:border-cyan-400 focus:ring-2 focus:ring-cyan-500/50 transition-all"
          />
          <button
            onClick={handleSend}
            className="p-3 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-full hover:shadow-lg hover:shadow-cyan-500/40 transition-all duration-300 transform hover:scale-105"
          >
            <PaperAirplaneIcon />
          </button>
        </div>
